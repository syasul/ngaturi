...
export const Testimonials: React.FC = () => {
  ...
  // Double the reviews array to create seamless infinite scrolling effect
  ...
  return (
    <section id="testimonials" className="py-24 bg-white overflow-hidden">
      ...
      <div className="relative w-full flex items-center overflow-hidden py-4 select-none">
        {/* Left and Right Blur overlays for premium fade effect */}
        ...
        <div className="flex gap-6 animate-marquee hover:[animation-play-state:paused] w-max">
          {doubleReviews.map((rev, i) => (
            <Card
              key={i}
              hoverable={false}
              className="w-[280px] sm:w-[350px] shrink-0 border border-gray-100 p-6 sm:p-8 flex flex-col justify-between"
            >
              <div>
                {/* Rating stars */}
                ...
                <p className="font-sans text-sm sm:text-base text-charcoal/70 leading-relaxed italic mb-6">
                  "{rev.text}"
                </p>
              </div>

              <div className="flex items-center gap-3 border-t border-gray-50 pt-4">
                <div className="w-10 h-10 bg-gold-50 border border-gold-100 rounded-full flex items-center justify-center font-serif text-sm font-bold text-gold-600">
                  {rev.couple.charAt(0)}
                </div>
                <div>
      ...
      {/* Add Marquee CSS animation dynamically inside tailwind or index.css */}
    </section>
  )
}
