...
export const Guests: React.FC = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Hapus ${selectedIds.length} tamu terpilih?`)) return
  ...
  const handleImportTextSubmit = async (e: React.FormEvent) => {
    ...
    if (parsedGuests.length === 0) {
      toast.error('Format data tidak valid. Pastikan format: Nama, No Telp')
      return
    }

    try {
      const res = await api.post('/guests/import', { guests: parsedGuests })
      if (res.data.status === 'success') {
        toast.success(res.data.message)
  ...
  return (
    <div className="space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-sand/35 pb-4">
        <div>
          <h2 className="text-3xl font-serif font-bold text-charcoal">Manajemen Tamu Undangan</h2>
          <p className="text-sm text-charcoal/60 mt-1">
            Undang tamu, buat tautan kustom, kirim melalui WhatsApp, dan pantau status kehadiran.
          </p>
        </div>

        <div className="flex flex-wrap gap-2.5">
          <Button variant="outline" size="md" className="flex items-center gap-2" onClick={() => setIsImportModalOpen(true)}>
            <Upload size={16} />
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white border border-sand/45 rounded-3xl p-4 shadow-sm">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          {selectedIds.length > 0 && (
            <Button variant="danger" size="sm" className="flex items-center gap-2 w-full sm:w-auto" onClick={handleBulkDelete}>
              <Trash2 size={14} />
      ...
      {/* Guest Table */}
      <Card className="overflow-x-auto border border-sand/45 bg-white shadow-sm rounded-3xl p-2">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-2">
            <Loader2 className="animate-spin text-gold-500" size={32} />
            <p className="text-xs text-charcoal/60">Memproses data tamu...</p>
          </div>
        ) : guests.length === 0 ? (
          <div className="py-20 text-center text-charcoal/40 text-sm">
            <Users className="mx-auto text-charcoal/20 mb-3" size={36} />
            <p>Daftar tamu kosong.</p>
            <p className="text-xs text-charcoal/30 mt-1">Mulai tambahkan tamu atau gunakan fitur import.</p>
          </div>
        ) : (
            ...
            <tbody>
              {guests.map((g) => {
                ...
                return (
                  <tr
                    key={g.id}
                    className={`border-b border-sand/20 hover:bg-cream/10 transition-colors ${
                  ...
                  >
                    ...
                    <td className="p-3 font-semibold text-charcoal">{g.name}</td>
                    <td className="p-3 text-charcoal/70">{g.phone || '-'}</td>
                    <td className="p-3">
                      <Badge
                        variant={
                          g.rsvpStatus === 'attending' || g.rsvpStatus === 'hadir'
                            ? 'success'
                            : g.rsvpStatus === 'declined' || g.rsvpStatus === 'tidak hadir'
                            ? 'danger'
                            : g.rsvpStatus === 'tentative' || g.rsvpStatus === 'ragu-ragu'
                            ? 'warning'
                            : 'neutral'
                        }
                      >
                    ...
                    <td className="p-3 text-xs text-gold-600 truncate max-w-xs">
                      {getGuestInvitationLink(g.uniqueToken)}
                    </td>
                    <td className="p-3 text-right flex justify-end gap-1.5">
                      <button
                        onClick={() => handleSendWA(g)}
                        title="Kirim Undangan WA"
                        className="p-1.5 rounded-lg bg-green-50 text-green-600 border border-green-200 hover:bg-green-100 cursor-pointer"
                      >
                        <MessageSquare size={14} />
                      </button>
                      <button
                        onClick={() => handleCopyLink(g.uniqueToken)}
                        title="Salin Link"
                        className="p-1.5 rounded-lg bg-gold-50 text-gold-600 border border-gold-200 hover:bg-gold-100 cursor-pointer"
                      >
                        <Copy size={14} />
                      </button>
                      <button
                        ...
                        title="Edit Tamu"
                        className="p-1.5 rounded-lg bg-cream border border-sand hover:bg-cream/50 text-charcoal/70 cursor-pointer"
                      >
                        <Edit2 size={14} />
                      </button>
                      <button
      ...
      <Card className="p-6 border border-sand/45 bg-white shadow-sm rounded-3xl space-y-4">
        <h3 className="text-lg font-serif font-bold text-charcoal flex items-center gap-1.5">
          <Sparkles className="text-gold-500" size={18} />
          <span>Kustomisasi Template Pesan WhatsApp</span>
        </h3>
        <p className="text-xs text-charcoal/50">
          Gunakan variabel <code className="bg-cream/50 px-1 py-0.5 rounded font-mono font-bold text-gold-600">{`{nama}`}</code> dan <code className="bg-cream/50 px-1 py-0.5 rounded font-mono font-bold text-gold-600">{`{link}`}</code> untuk diganti otomatis saat mengirim pesan undangan.
        </p>
        <textarea
          rows={5}
          value={waTemplate}
          onChange={(e) => setWaTemplate(e.target.value)}
          className="w-full border border-sand bg-white px-3 py-2 rounded-xl text-sm focus:ring-1 focus:ring-gold-500"
        />
      </Card>

      {/* MODAL: ADD GUEST */}
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Tambah Tamu Manual">
        <form onSubmit={handleAddGuest} className="space-y-4 font-sans">
          <div>
            <label className="block text-xs font-bold text-charcoal/60 uppercase">Nama Lengkap Tamu</label>
            <input
              type="text"
              required
              value={guestName}
          ...
          <div>
            <label className="block text-xs font-bold text-charcoal/60 uppercase">No. WhatsApp (Opsional)</label>
            <input
              type="text"
              value={guestPhone}
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Data Tamu">
        <form onSubmit={handleEditGuest} className="space-y-4 font-sans">
          <div>
            <label className="block text-xs font-bold text-charcoal/60 uppercase">Nama Lengkap Tamu</label>
            <input
              type="text"
      ...
      {/* MODAL: IMPORT TAMU */}
      <Modal isOpen={isImportModalOpen} onClose={() => setIsImportModalOpen(false)} title="Import Tamu dari Data Excel / CSV">
        <form onSubmit={handleImportTextSubmit} className="space-y-4 font-sans">
          <p className="text-xs text-charcoal/70">
            Salin baris-baris dari Excel Anda dan tempel di bawah. Gunakan tanda koma (<code className="bg-cream p-0.5 font-mono">,</code>) sebagai pemisah nama dan no telepon.
          </p>
          <div className="bg-cream/40 p-3 border border-sand/40 rounded-xl text-[10px] text-charcoal/50 leading-relaxed font-mono">
            Format input per baris:<br />
            Nama Tamu, No Telepon<br /><br />
            Contoh:<br />
            Joni Wijaya, 08122334455<br />
          ...
          <div>
            <textarea
              ...
              className="w-full border border-sand bg-white px-3 py-2 rounded-xl text-xs focus:ring-1 focus:ring-gold-500 font-mono"
            />
          </div>
          <div className="flex gap-2 justify-end mt-4">
            <Button variant="ghost" type="button" onClick={() => setIsImportModalOpen(false)}>
              Batal
            </Button>
            <Button variant="primary" type="submit">
              Proses Import
