...
const CustomTheme1Countdown = ({ targetDate }: { targetDate: string }) => {
...
interface CustomTheme1Props extends ThemeProps {
    isOpened?: boolean;
    onOpen?: () => void;
...
export const CustomTheme1: React.FC<CustomTheme1Props> = ({
    data,
    guestName,
    guestToken,
    photos,
                            ...
                            {(isOpening, handleOpen) => (
                                <>
                                    ...
                                    <div className="relative z-10 flex h-full w-full flex-1 flex-col items-center justify-between">
                                        ...
                                        {/* 2. TEKS/LOGO DI LAYER DEPAN (z-20 relative) */}
                        ...
                        >
                            ...
                            >
                                <div className="relative z-10 mt-0 flex w-full flex-col items-center justify-center space-y-2 sm:space-y-3">
                                    <CustomTheme1Countdown targetDate={weddingDate} />
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="relative z-10 mx-auto w-fit space-y-10 px-6 pb-20 pt-[45%] sm:px-10">
                                    {(() => {
                                        const getTimelineIcon = (idx: number, iconName?: string) => {
                                            const icons = [
                                                "/assets/theme_1/outline-kursi.webp",
                                                "/assets/theme_1/pengantin-outline.webp",
                                                "/assets/theme_1/outlinecincin.webp",
                                                "/assets/theme_1/potongan-tangan.webp",
                                                "/assets/theme_1/pengantin-ciuman-outline.webp",
                                                "/assets/theme_1/outline-foto.webp"
                                            ];
                                            if (iconName) {
                                                const norm = iconName.toLowerCase();
                                                if (norm.includes('kursi') || norm.includes('chair') || norm.includes('tamu') || norm.includes('welcome')) return "/assets/theme_1/outline-kursi.webp";
                                                if (norm.includes('pengantin') || norm.includes('bride') || norm.includes('groom') || norm.includes('couple') || norm.includes('datang')) return "/assets/theme_1/pengantin-outline.webp";
                                                if (norm.includes('cincin') || norm.includes('ring') || norm.includes('akad') || norm.includes('nikah')) return "/assets/theme_1/outlinecincin.webp";
                                                if (norm.includes('tangan') || norm.includes('hand') || norm.includes('ucap') || norm.includes('selamat')) return "/assets/theme_1/potongan-tangan.webp";
                                                if (norm.includes('ciuman') || norm.includes('kiss') || norm.includes('resepsi') || norm.includes('party')) return "/assets/theme_1/pengantin-ciuman-outline.webp";
                                                if (norm.includes('foto') || norm.includes('photo') || norm.includes('camera') || norm.includes('bersama')) return "/assets/theme_1/outline-foto.webp";
                                            }
                                            return icons[idx % icons.length];
                                        };

                                        const timelineItems = data?.timeline || data?.rundown || [
                                        ...
                                        return timelineItems.map((item: any, idx: number) => (
                                            <div key={idx} className="flex items-center gap-4 sm:gap-6">
                                                ...
                                                <div className="flex-1 space-y-1 text-left">
                                                    ...
                                                    <p
                                                        className="text-[11px] font-semibold leading-snug sm:text-xs whitespace-pre-line"
                                                        style={{ color: primaryColor }}
                                                    >
                                                        {item.title}
                                                    </p>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            {customStyle.ewalletNumber && (
                                                <div className="rounded-[1.25rem] border border-[#7A223E]/15 bg-gradient-to-br from-white/90 to-[#FCF0F2]/90 p-6 text-left shadow-[0_4px_20px_-10px_rgba(122,34,62,0.15)] backdrop-blur-md">
                                                    <div className="mb-6 flex items-center justify-between">
                                                        <div className="flex items-center gap-4">
                                                            <div className="flex h-5 w-8 items-center justify-center rounded-[3px] bg-[#6B1D2F] text-white p-[3px]">
                                                        ...
                                                        <span className="text-[9px] font-bold uppercase tracking-widest text-[#7A223E]/50">
                                                            DOMPET DIGITAL
                                                        </span>
                                                    </div>
                                            ...
                                            {customStyle.qrisUrl && (
                                                <div className="flex flex-col items-center justify-center rounded-[1.25rem] border border-[#7A223E]/15 bg-gradient-to-br from-white/90 to-[#FCF0F2]/90 p-6 text-center shadow-[0_4px_20px_-10px_rgba(122,34,62,0.15)] backdrop-blur-md">
                                                    {customStyle.ewalletNumber && (
                                                        <p className="mb-4 font-serif text-[9px] font-bold text-[#7A223E]/70 uppercase tracking-widest">
                                ...
                                <div className="flex flex-col gap-6 font-serif text-2xl italic text-[#FAF3EC]">
                                    ...
                                    {(guestName || guestToken) && (
                                        <button
                                            ...
                                            className="flex items-center justify-center gap-2 rounded-2xl border border-[#C9A84C]/40 bg-[#C9A84C]/10 px-4 py-2.5 text-base font-sans font-bold tracking-wide text-[#C9A84C] transition-all hover:bg-[#C9A84C]/20"
                                        >
                                            <QrCode size={18} />
                                            <span>QR Kehadiran Tamu</span>
                                        </button>
                                    )}
                                </div>
                            </div>
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="space-y-6">
                                    ...
                                    {/* E-Wallet Card */}
                                    {customStyle.ewalletNumber && (
                                        <div className="rounded-[1rem] border border-[#7A223E]/15 bg-white p-4 text-left shadow-sm">
                                            <div className="mb-2 flex items-center justify-between text-[11px] font-bold text-[#6B1D2F]">
                                                <span>{customStyle.ewalletName || 'E-Wallet'}</span>
                                                <span className="text-[7.5px] font-bold uppercase tracking-widest text-[#7A223E]/50">
                                                    Dompet Digital
                                                </span>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <p className="font-sans text-[1.15rem] font-bold text-[#6B1D2F] tracking-wide">
                                                    {customStyle.ewalletNumber}
                                                </p>
                                                <button
                                                    onClick={() => handleCopyEWallet(customStyle.ewalletNumber)}
                                                    className="flex items-center justify-center gap-1 rounded-full border border-[#7A223E]/20 px-2.5 py-1 text-[7px] font-bold text-[#6B1D2F] transition-all hover:bg-neutral-50"
                                                >
                                                    {copiedEWallet ? 'COPIED' : 'SALIN'}
                                                </button>
                                            </div>
                                            {customStyle.ewalletHolderName && (
                                                <p className="mt-1 font-serif text-[9px] font-bold text-[#6B1D2F]">
                                                    a.n {customStyle.ewalletHolderName}
                                                </p>
                                            )}
                                        </div>
                                    ...
                                    {!customStyle.qrisUrl && !customStyle.ewalletNumber && (
                                        <div className="relative mx-auto flex aspect-square w-full max-w-[180px] items-center justify-center overflow-hidden rounded-[1rem] border border-[#7A223E]/10 bg-white p-4 shadow-inner">
                                            <QrCode
                                                size={120}
