...
export const AdminSettings: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<
        'smtp' | 'tripay' | 'midtrans' | 'storage' | 'telegram' | 'maintenance'
    >('smtp');
                ...
                if (response.data.settings) {
                    ...
                    setSettings({
                        smtp: {
                            ...
                            port: String(
                                response.data.settings.smtp?.port || '587',
                            ),
                        ...
                        tripay: {
                            ...
                            privateKey:
                                response.data.settings.tripay?.privateKey || '',
                            merchantCode:
                                response.data.settings.tripay?.merchantCode ||
                                '',
                            isSandbox:
                                response.data.settings.tripay?.isSandbox !==
                                false,
                        },
                        midtrans: {
                            clientKey:
                                response.data.settings.midtrans?.clientKey ||
                                '',
                            serverKey:
                                response.data.settings.midtrans?.serverKey ||
                                '',
                            isProduction:
                                !!response.data.settings.midtrans?.isProduction,
                        },
                        storage: {
                            endpoint:
                                response.data.settings.storage?.endpoint || '',
                            accessKeyId:
                                response.data.settings.storage?.accessKeyId ||
                                '',
                            secretAccessKey:
                                response.data.settings.storage
                                    ?.secretAccessKey || '',
                            bucket:
                                response.data.settings.storage?.bucket || '',
                        },
                        telegram: {
                            enabled: !!response.data.settings.telegram?.enabled,
                            botToken:
                                response.data.settings.telegram?.botToken || '',
                            chatId:
                                response.data.settings.telegram?.chatId || '',
                        },
                        maintenance: {
                            enabled:
                                !!response.data.settings.maintenance?.enabled,
                            message:
                                response.data.settings.maintenance?.message ||
                                'Kami sedang melakukan pemeliharaan rutin. Silakan kembali lagi beberapa saat lagi.',
                        },
                    });
                }
            } catch (err: any) {
    ...
    const handleTestEmail = async () => {
        ...
        try {
            ...
            toast.success(
                response.data.message || 'Test email berhasil dikirim!',
            );
        } catch (err: any) {
            toast.error(
                err.response?.data?.message || 'Gagal mengirim email uji coba.',
            );
        } finally {
        ...
        <div className="space-y-8">
            <div>
                ...
                <p className="text-sm text-slate-400">
                    Kelola API key, kredensial SMTP, dan parameter sistem
                    Ngaturi
                </p>
            </div>
            ...
            <div className="flex flex-col items-start gap-8 lg:flex-row">
                {/* Sidebar Nav */}
                <div className="w-full space-y-1 rounded-2xl border border-slate-800 bg-slate-900 p-4 lg:w-64">
                    {tabs.map((tab) => {
                        const Icon = tab.icon;
                        ...
                        return (
                            <button
                                ...
                                className={`flex w-full cursor-pointer items-center gap-3 rounded-xl px-4 py-2.5 text-left text-xs font-semibold transition-all ${
                                    isActive
                ...
                {/* Tab Form Content */}
                <div className="relative w-full flex-1 rounded-2xl border border-slate-800 bg-slate-900 p-6 sm:p-8">
                    <form onSubmit={handleSave} className="space-y-6">
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">
                                        Konfigurasi server email untuk OTP dan
                                        tagihan lunas
                                    </p>
                                </div>
                                ...
                                {/* Email tester widget */}
                                <div className="border-slate-850 mt-6 rounded-2xl border border-t border-slate-800 bg-slate-950/40 p-5 pt-6">
                                    <h4 className="text-slate-350 mb-1 text-xs font-semibold">
                                        Kirim Email Uji Coba (Tester)
                                    </h4>
                                    <p className="mb-3 text-[10px] text-slate-500">
                                        Uji konfigurasi SMTP di atas dengan
                                        mengirim email tes ke alamat pribadi.
                                    </p>
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            ...
                                            className="flex cursor-pointer items-center gap-1.5 rounded-xl border border-slate-700 bg-slate-800 px-4 text-xs font-bold text-slate-300 transition-all hover:bg-slate-700"
                                        >
                                            {sendingTest ? (
                                                <Loader2
                        ...
                        {activeTab === 'tripay' && settings.tripay && (
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">
                                        Integrasi merchant payment gateway
                                        otomatis (direkomendasikan untuk startup
                                        Indonesia)
                                    </p>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-center justify-between border-t border-slate-800/80 pt-4">
                                        <div>
                                            <span className="text-slate-350 block text-xs font-semibold">
                                                Sandbox Mode (Uji Coba)
                                            </span>
                                            <span className="block text-[10px] text-slate-500">
                                                Aktifkan untuk integrasi
                                                simulator Tripay Sandbox
                                            </span>
                                        </div>
                                        <button
                                            type="button"
                                    ...
                                    <div className="border-t border-slate-800/80 pt-4">
                                        ...
                                        <span className="mt-0.5 block text-[10px] text-slate-500">
                                            Konfigurasikan callback URL berikut
                                            pada panel merchant Tripay Anda:
                                        </span>
                                        <div className="border-slate-850 mt-2 select-all rounded-xl border bg-slate-950 p-3 font-mono text-xs text-amber-500">
                        ...
                        {activeTab === 'midtrans' && settings.midtrans && (
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">
                                        Integrasi pembayaran tagihan invoice
                                        otomatis
                                    </p>
                                </div>
                        ...
                        {activeTab === 'telegram' && settings.telegram && (
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">
                                        Kirim notifikasi transaksi/order masuk
                                        langsung ke channel admin
                                    </p>
                                </div>
                            ...
                            settings.maintenance && (
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500">
                                            Kunci seluruh akses platform untuk
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <textarea
                                                ...
                                                rows={4}
                                                className="block w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-2.5 font-poppins text-sm text-slate-100 focus:outline-none focus:ring-1 focus:ring-amber-500"
                                            />
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="mt-8 flex justify-end border-t border-slate-800 pt-6">
                            <Button
                                type="submit"
                                disabled={saving}
                                className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-amber-500 px-6 py-2.5 text-sm font-bold text-slate-950 transition-all hover:bg-amber-600 sm:w-auto"
                            >
                                {saving ? (
                                    <Loader2
                                        className="animate-spin"
